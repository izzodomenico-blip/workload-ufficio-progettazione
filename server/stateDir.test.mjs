...
import { ADMIN_CONFIG_PATH } from './services/adminAuth.js'
import { CONSUNTIVI_CONFIG_PATH } from './services/consuntiviAuth.js'
...
describe('config password di sezione derivati da STATE_DIR', () => {
  it('ADMIN_CONFIG_PATH sotto STATE_DIR', () => {
    expect(ADMIN_CONFIG_PATH).toBe(path.join(STATE_DIR, 'admin.config.json'))
  })
  it('CONSUNTIVI_CONFIG_PATH sotto STATE_DIR', () => {
    expect(CONSUNTIVI_CONFIG_PATH).toBe(path.join(STATE_DIR, 'consuntivi.config.json'))
  })
})
